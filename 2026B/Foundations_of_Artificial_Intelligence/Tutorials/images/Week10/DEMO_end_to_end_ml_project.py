...
# 4.4.3 Define MyFeatureAdder: a transformer for adding features "TOTAL NUMBER OF ROOMS",...  
...
if 10: 
    ...
    columns_header = train_set.columns.tolist() + ["TOTAL NUMBER OF ROOMS"] + onehot_cols
...
if type(best_model).__name__ == "RandomForestRegressor":
    ...
    for val_list in full_pipeline.transformer_list[1][1].named_steps['cat_encoder'].categories_: 
        onehot_cols = onehot_cols + val_list.tolist()
    feature_names = train_set.columns.tolist() + ["TOTAL NUMBER OF ROOMS"] + onehot_cols
    for name in cat_feat_names:
