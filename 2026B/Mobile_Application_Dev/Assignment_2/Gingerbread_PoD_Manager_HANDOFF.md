...
### 6.1 Sprint 0 — Setup & Architecture · tag `v0.1-skeleton`
...
- Version ban đầu: Kotlin 2.2.10, KSP 2.2.10-2.0.2, Hilt 2.59.2 (2.59.0/2.59.1 lỗi `ComponentTreeDeps` với AGP 9), Maps Compose 6.4.4 — **đã thay đổi ở Sprint 1**.
...
- Tạo `develop` từ `main` sau tag. Branch protection **chưa bật**.

### 6.2 Sprint 1 — Core Feature Build · tag dự kiến `v0.2-core-features`
...
- UC-04 (D): `SyncStatus`, `SyncQueueItem`, `SyncQueueEntity/Dao`, `SyncRepository`, `DeliveryUploader`, `PdfGenerator`, 2 use case, `SyncWorker`, `NetworkStateReceiver`, `AndroidPdfGenerator`, `StubDeliveryUploader`, `SyncModule`, Dashboard, `ExportDeliveryCertificateUseCase`, 5 test.
...
- Thực tế repo cuối Sprint 1: 19 unit test, chưa có UI test, `Screen.Pod` vẫn route tĩnh + `PLACEHOLDER_DELIVERY_ID`.

### 6.3 Sprint 2 — Integration · tag dự kiến `v0.3-integrated`

- Nhánh `sprint2/uc0x-integration`. Thứ tự merge: **A → C → B → D** (C trước B được vì C dùng interface của B + fallback).
- ⚠️ Audit PR #9 (A) phát hiện 2 lỗi kiến trúc:
...
### 6.4 Sprint 3 — Testing, Hardening & Submission

Quyết định **không** làm Retrofit thật và backend upload.

1. Compose UI test `SignatureCaptureScreenTest` (sign → save → reset; clear)
2. Google Fonts + font certs; palette Style 1 (Outfit/Inter); fix chữ OCR Scan tràn dòng, padding nút
3. Xoá test mẫu mặc định của Android Studio; splash screen thương hiệu
4. Elapsed time + destination sống qua chuyển tab; fix tràn layout Dashboard; PDF lưu Downloads
5. `DeliveryDao` REPLACE → `@Upsert` (hết mất ảnh PoD); EXIF; PDF ra Downloads công khai
6. Đổi `app_name` → "Gingerbread PoD"; đổi icon app
7. Camera Tracking: overview 1 lần rồi follow zoom 17
...
15. `feature_auth`: Login 2 role + DataStore + tách ShipperFlow/ManagerFlow + Logout + gate notification
16. "button size" — nút role 0.25f → **0.15f** chiều cao màn hình
17. `updateDestination()` — đồng bộ đích đến theo stop đang active khi chuyển tab (⚠️ làm gãy compile unit test, xem §9 mục 1)

### 6.5 Thay đổi đáng chú ý so với kế hoạch gốc
